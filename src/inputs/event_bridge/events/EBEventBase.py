...
class EBEventBase (EventI):
    ...
    def is_valid(cls, event: dict) -> bool:
        if (event.get("source", None) != "aws.events" or
                event.get("resources", None) is None or
                not isinstance(event.get("resources"), list) or
                event.get("resources")[0] != "arn:aws:events:rule/{}".format(cls.get_event_type())):
            return False
        return True